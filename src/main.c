...
void
encrypt (unsigned char *tun_buf, unsigned char *udp_buf, size_t buf_sz)
{
  int i;
  unsigned char xor_number;
  for (i = 0; i < buf_sz; i++)
    {
      xor_number = tun_buf[i] ^ MAGIC_CRYPT_NUMBER;
      udp_buf[i] = xor_number;
    }
}
void
decrypt (unsigned char *udp_buf, unsigned char *tun_buf, size_t buf_sz)
{
  int i;
  unsigned char decrypt_number;
  for (i = 0; i < buf_sz; i++)
    {
      decrypt_number = udp_buf[i] ^ MAGIC_CRYPT_NUMBER;
      tun_buf[i] = decrypt_number;
    }
}
